...
Jotline needs Python 3.11+ and a Unicode/color terminal. Download the wheel from
the [latest GitHub release](https://github.com/CaulShiver/jotline/releases/latest).
...
```sh
# Linux
sha256sum jotline-0.9.4-py3-none-any.whl
# macOS
shasum -a 256 jotline-0.9.4-py3-none-any.whl
```

In PowerShell, run
`Get-FileHash ./jotline-0.9.4-py3-none-any.whl -Algorithm SHA256`.
...
Both commands also work in PowerShell. If the command is not on PATH, follow
uv's `uv tool update-shell` or pipx's `pipx ensurepath` instructions and reopen
your terminal. These tools install Jotline in an isolated environment.
...
# Linux / macOS
jotline-env/bin/python -m pip install ./jotline-0.9.4-py3-none-any.whl
...
```sh
uv tool install --force ./jotline-0.9.4-py3-none-any.whl
# Or, if installed with pipx:
pipx install --force ./jotline-0.9.4-py3-none-any.whl
jotline --version
jotline doctor
```
