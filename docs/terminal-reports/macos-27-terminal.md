...
| Copy with clipboard enabled | pass | After native `pbcopy` copy, Ctrl+P → Copy note. `pbpaste` matched the synthetic body `OSC52-NATIVE-BODY-pbcopy-fix` in Terminal.app 2.15. OSC 52 is still blocked here; macOS copy no longer depends on it. |
| Copy with OSC 52 disabled (request, not success) | pass | Terminal.app still blocks OSC 52. Native copy succeeded anyway. When `pbcopy` is unavailable, Jotline still describes OSC 52 as a request (`tests/test_clipboard.py`). |
