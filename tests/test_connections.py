...
from jotline.links import iter_wiki_links, wiki_link_at, wiki_link_targets
...
async def test_broken_link_can_create_a_note(tmp_path):
    vault = Vault(tmp_path)
    app = Jotline(vault)
    async with app.run_test(size=(100, 35)) as pilot:
        editor = app.query_one("#editor", TextArea)
        ...
        await pilot.press("enter")
        await pilot.pause()
        assert app.current.title == "Ghost idea"
        assert vault.backlinks(app.current)


async def test_compact_status_keeps_connections_reachable(tmp_path):
    vault = Vault(tmp_path)
    target = vault.new("# Target")
    vault.save(target)
    source = vault.new(f"# Source\n[[{target.id}|Target]]")
