...
SCRYPT_R = 8
SCRYPT_P = 1
# Bound combined costs, not just each untrusted parameter. The default uses
# 128 MiB; allow up to 256 MiB and four times its CPU work for compatible keys.
...
def _derive(passphrase: str, salt: bytes, n: int, r: int, p: int) -> bytes:
    ...
    # Fixed headroom covers the small p-dependent overhead and stays below
    # the backend's signed 32-bit maxmem limit for every accepted parameter set.
